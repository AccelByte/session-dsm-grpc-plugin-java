...
package net.accelbyte.session.sessiondsm.service.gcp;
...
public class SessionDsm extends SessionDsmGrpc.SessionDsmImplBase {
    ...
    @Autowired
    public SessionDsm(AccelByteSDK sdk) throws IOException {
        this.sdk = sdk;
        ...
        this.instancesClient = InstancesClient.create(
                InstancesSettings.newBuilder()
                        .setCredentialsProvider(() -> GoogleCredentials
                                .fromStream(new FileInputStream(config.getGcpServiceAccountFile())))
                        .build());
        ...
        log.info("SessionDsm GCP service initialized");
    }
    ...
    public void createGameSession(RequestCreateGameSession request,
        ...
        try {
            ...
            responseObserver.onError(new StatusRuntimeException(
                    Status.INTERNAL.withDescription("Failed to create instance: " + e.getMessage())));
        }
    }
    ...
    public void terminateGameSession(RequestTerminateGameSession request,
        ...
        String instanceName = request.getNamespace() + "-" + request.getSessionId();
        try {
            OperationFuture<Operation, Operation> operation = instancesClient.deleteAsync(config.getGcpProjectId(),
                    request.getZone(), instanceName);
            operation.get(300, TimeUnit.SECONDS);
            ...
            responseObserver.onError(new StatusRuntimeException(
                    Status.INTERNAL.withDescription("Failed to terminate instance: " + e.getMessage())));
        }
    }
